...
def random_generate_data(num_points, ptimes = 2, data_scale = 10.0):
    ...
    x = random_numbers_scaled * data_scale
    y = polynomial_function(x, ptimes)

    return x, y

def common_generate_data(num_points, ptimes = 2, data_scale = 10.0):
    x = torch.linspace(-1, 1, num_points) * data_scale
    y = polynomial_function(x, ptimes)
    return x, y
